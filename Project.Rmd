...
```{r, fig.dim=c(2,2)}
par(mfrow=c(1,2)) # PROBLEM
plot.PCA(res.acp, choix="var", axes = c(1,2), new.plot = FALSE, title = "Graphe des variables (axes 1 et 2)")
plot.PCA(res.acp, choix="var", axes = c(1,3), new.plot = FALSE, title = "Graphe des variables (axes 1 et 3)")
plot.PCA(res.acp, choix="var", axes = c(2,3), new.plot = FALSE, title = "Graphe des variables (axes 2 et 3)")
```
...
```{r}
plot(res.acp, choix="ind", axes = c(1,2), invisible="quali", habillage="Energy.efficiency", label = "none")
plot(res.acp, choix="ind", axes = c(2,3), invisible="quali", habillage="Energy.efficiency", label = "none")
plot(res.acp, choix="ind", axes = c(1,3), invisible="quali", habillage="Energy.efficiency", label = "none")

# plot(res.acp, choix="ind", axes = c(1,2), invisible="quali", habillage="Glazing.area.distr", label = "none")
# plot(res.acp, choix="ind", axes = c(2,3), invisible="quali", habillage="Glazing.area.distr", label = "none")
# plot(res.acp, choix="ind", axes = c(1,3), invisible="quali", habillage="Glazing.area.distr", label = "none")
# 
# plot(res.acp, choix="ind", axes = c(1,2), invisible="quali", habillage="orientation", label = "none")
# plot(res.acp, choix="ind", axes = c(2,3), invisible="quali", habillage="orientation", label = "none")
# plot(res.acp, choix="ind", axes = c(1,3), invisible="quali", habillage="orientation", label = "none")
```

On peut voir sur le premier et le troisième graphique que l'axe 1 marque une claire séparation entre les classes A, B, C et les classes D, E, F, G. Les maisons dont la consommation énergétique la plus faible possède des coordonnées négatives pour la première composante. Ces maisons sont celles qui ont une compacité relative plus faible, plus basses et avec une surface au sol plus élevée, comme on peut le voir sur les graphiques suivants :
...
# grid.arrange(rbind(gg1, gg2, gg3,)) 
# grid.arrange(grobs = list(gg1,gg2,gg3),ncol = 2, widths=c(10,10))
...
# selectf = regsubsets(Energy ~ (. - Roof.area)^2, data = data[,quanti],nbest=1,nvmax=10,method="forward")
# modselect_bic=stepAIC(model_quanti_complet,trace=TRUE,direction=c("backward"),k=log(nrow(data)))
...
# model1f = lm(Energy ~ (. - Roof.area)^2 - Overall.height - Glazing.area - Relative.compactness:Glazing.area
#                                      - Surface.area:Glazing.area - Wall.area:Glazing.area, data = data)
# model0f = lm(Energy ~ (. - Roof.area)^2 - Overall.height - Glazing.area - Relative.compactness:Glazing.area
#                                      - Relative.compactness:Wall.area - Surface.area:Glazing.area - Wall.area:Glazing.area, data = data)
# 
# selectb = regsubsets(Energy ~ (. - Roof.area)^2, data = data[,quanti],nbest=1,nvmax=10,method="backward")
# plot(selectb,scale="bic",main="Backward BIC", cex.axis = 0.5)
# plot(selectb,scale="Cp",main="Backward Cp")
# plot(selectb,scale="adjr2",main="Backward adjusted R²")
# model1b = lm(Energy ~ (. - Roof.area)^2 - Overall.height - Surface.area:Wall.area - Surface.area:Glazing.area
#                                      - Wall.area:Overall.height - Overall.height:Glazing.area, data = data)
# model1b = lm(Energy ~ (. - Roof.area)^2 - Relative.compactness - Overall.height - Surface.area:Wall.area - Surface.area:Glazing.area
#                                      - Wall.area:Overall.height - Overall.height:Glazing.area, data = data)
```
