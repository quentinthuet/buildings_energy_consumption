...
scinder notre jeu de données en train et test.
```{r}
...
Nous allons tracé la  table de contingence par rapport aux données prédites et réelles dans l'ensemble test.
```{r}
...
library(rpart.plot)
```
Pour la création de l'arbre binaire de décision, nous allons  utiliser la fonction de rpart avec les paramètres par
défaut.
```{r}
tree.dis=rpart(Energy.efficiency.bis~.,data=train.dis, parms = list(split = "information"),cp=0.001)
rpart.plot(tree.dis)
```
L'analyse de l'arbre binaire de décision permet d'avoir une classification en deux catégories selon la variables Overall.height.
Dans la deuxième catégorie où l'Overall.height=7,c'est la variable Wall.area qui est utilisé pour scinder l'arbe. Cependant on observe  que la profondeur de la sous arbre est grande par à l'autre sous catégorie. Ainsi, on en déduit que l'arbre pourrait manquer de flexibilité pour de 
nouvelles données.
...
#### Version optimale
L'arbre de décision binaire a pour défaut  de ne pas être flexible pour la prédiction de nouvelle variable. Pour palier à ce problème, nous
...
```
L'arbre optimal obtenu comporte bien moins de feuilles que ce celui non optimal, ce qui signe d'une structure plus flexible.
```{r}
...
pct_arb_opti_dis
```
Après avoir réalisé la table de contingences sur les données testées, on observe 90,10 % d'individus bien classé.
### Random forest
