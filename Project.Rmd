...
```{r}
# On réalise un jeu de test et un jeu d'entrainement qui nous servirons tout le long de notre étude (voir conclusion)
ratio_train = 0.75
set.seed(1999)
...
La p-valeur obtenue est de 0.003229, donc on rejette le sous modèle BIC au risque de 1\%.
...
```{r}
# utile pour la conclusion
final_logistic_model = glm(mlg_best_aic$formula, data = data.mlg[train_ind,],family=binomial(link="logit"))
```
...
Pour la réalisation de l'arbre optimal, de la régression logistique et du random forest. Nous allons
scinder notre jeu de données en ensemble d'apprentissage et en ensemble de test. Cela nous permettra
...
### Régression logistique
Dans la partie modèle linéaire généralisé, nous avions déjà construit un modèle de régression logistique. A partir de ce modèle, nous allons tracé la  table de contingence pour les éléments constituants l'ensemble test.
```{r}
...
pct_reg_log
```
On retrouve un pourcentage de valeurs corectement prédites de 90,625% pour la régression logistique.
...
library(rpart.plot)
```
Pour la création de cet arbre, nous allons  utiliser la fonction de rpart avec les paramètres par
défaut.
```{r}
tree.dis=rpart(Energy.efficiency.bis~.,data=train.dis, parms = list(split = "information"),cp=0.001)
rpart.plot(tree.dis)
```
L'analyse de l'arbre binaire de décision permet d'avoir une classification en deux catégories selon la variables Overall.height. Dans la première 
catégorie (Overall.height = 3.5), il n'y pas d'autre subdivision qui sont faites. On
Dans la deuxième catégorie où l'Overall.height=7, on observe  que la profondeur de la sous arbre est grande par à l'autre sous catégorie. Ainsi, on en déduit que l'arbre pourrait manquer de flexibilité pour de nouvelles données.
...
pct_arb_nai_dis
```
Après avoir traçé la table de contingence on retrouve un pourcentage 89,06% d'individus bien classés. 
#### Version optimale

L'arbre de décision binaire a pour défaut  de ne pas être flexible pour la prédiction de nouvelle variable. Pour pallier à ce problème, nous
allons faire la recherche de l'arbre optimal qui minimise la cross validation error.
```{r}
# Optimisation de cp
...
```
L'arbre optimal obtenu comporte bien moins de feuilles que ce celui non optimal, ce qui est un signe d'une structure plus flexible (car plus général). Cependant, le pourcentage trouvée sur les données en apprentissage va se voir diminuer.
```{r}
...
pct_arb_opti_dis
```
Après avoir réalisé la table de contingences sur les données testées, on observe 90,10 % d'individus bien classé, ce qui est supérieur à la valeur trouvée sur l'arbre non optimisée.
### Random forest
Dans cette partie nous allons contruire une forêt aléatoire, en vue de prédire l'appertance d'un individus à une classe. En effet, nous avons vue précedemment que l'arbre optimal pouvait être instable par rapport aux données d'apprentissage. Ainsi, grâce au bagging et la variable sampling qu'implémente l'algorithme de random forest nous devrions avoir une plus grande stabilité de notre algorithme.
```{r, echo=F}
library(randomForest)
```

#### Version naïve (``mtry`` par défaut)
Nous allons commencer par la réalisation d'un random forest avec les paramètres par défaut de la fonction randomForest.
```{r}
...
pct_rdm_nai_dis
```
On observe pour les paramètres par défaut du random forest un pourcentage de 92,18% 
#### Version optimale
Dans cette version, on cheche à optimiser le paramètre mtry qui permettrait d'avoir le meilleur arbre en minimisant l'erreur OOB.
```{r}
...
pct_rdm_opti_dis
```
On obtient le même pourcentage que précedemment, cela est peut-être dû à la stabilité de l'algorithme.
## Régression
...
D'après le graphique de l'importance des variables de la Random Forest, on retrouve la même hiérarchisation de la contribution des variables, à savoir Wall.area (deuxième axe de l'ACP), Surface.area et Relative.compactness (premier axe de l'ACP), puis Glazing.area (troisième axe de l'ACP).

# Conclusion

Nous avons ainsi pu mettre en place plusieurs méthodes de prédiction afin de prédire la consommation énergétique des bâtiments. Les résultats obtenus varient d'un modèle à l'autre, mais certaines tendances se dégagent tout de même.
...
L'influence la plus importante sur la consommation semble être clairement liée à la forme du bâtiment. Ainsi, nos modèle conservent toujours les variables `Relative.compactness` et `Overall.height`. Les variables `Surface.area` et `Wall.area` sont aussi souvent conservée, mais il y a de toute manière un lien entre toutes ces variables.
