---
title: "Projet de statistiques et d'analyse de données"
subtitle: "Prédiction de la consommation énergétique de bâtiments"
author: "CAMARA Ababacar, GÉRÉ Léo, THUET Quentin"
date: "29/01/2021"
output:
  pdf_document:
    toc: true
...
knitr::opts_chunk$set(echo = F, fig.align="center", results="hold", cache = TRUE, fig.dim=c(8,3))
library(ggfortify)
library(VGAM)
library(ROCR)
library(tidyverse)
...
et généralisés), ainsi que des modèles non linéaires (arbre optimal, forêts aléatoires).
...
et valeur 0 de `Glazing.area`) consomment moins d'énergie que les bâtiments vitrés, comme on s'y attend.
...
Les bâtiments à la consommation énergétique la plus faible possèdent des coordonnées positives pour la première composante. Ils sont plus bas et moins compacts, comme on peut le voir sur les graphiques suivants :
...
Ainsi, les maisons plus étalées sur le sol possèdent les meilleures performances énergétiques, de même que les maisons sans vitres.

```{r, fig.dim=c(6,2)}
plot(res.acp, choix="ind", axes = c(1,3), invisible="quali", habillage="Glazing.area.distr",
           label = "none", new.plot = FALSE) + theme(text = element_text(size=5))
```

On voit également une claire séparation des bâtiments sans vitre sur la troisième composante, qui correspondait à la surface vitrée.
...
Après cette analyse visuelle, nous allons utiliser des méthodes de clustering pour confirmer ou infirmer nos observations. Commençons par le clustering hiérarchique. Nous utiliserons le critère de Ward.
...
Sur le graphique de décroissance la variance inter-classe, on observe un saut important au passage à 2 classes. On retrouve également les résultats que l'on a obtenu avec le dendogramme en observant des sauts aux passages à 4, 5 et 12 classes.

Ces premiers résultats semblent confirmer les nombres de classes pertinents : 2, 4 et 12.
...
Après plusieurs exécutions pour 4 classes, les k-means ne donnent presque jamais les mêmes résultats que le clustering hiérarchique. Les clusters semblent souvent un peu moins pertinents (par exemple le groupe en haut à droite classé avec le groupe du milieu, assez éloigné).
...
Nous construisons d'abord naïvement un premier modèle contenant les variables quantitatives et leurs interactions.
...
Cependant, ce modèle utilise beaucoup de variables, on peut donc se demander si elles sont toutes pertinentes. Nous allons donc essayer de n'en conserver que certaines.

### Sélection de variables par critère BIC

Nous allons réaliser dans un premier temps une sélection de variables par critère BIC à l'aide de la commande `stepAIC`.
...
Le modèle selectionné conserve nos 5 variables quantitatives, mais supprime certaines de leur interactions. Afin de s'assurer que l'on peut simplifier notre modèle, on réalise un test de sous-modèle entre le modèle complet et le modèle selectionné.
...
Nous avons vu que la régression Ridge ne nous donnait pas de résultats très intéressants, il était difficile de voir clairement quelles variables écarter. Nous ne nous attarderons donc pas dessus, et nous allons plutôt essayer une régression lasso, qui permet d'écarter plus facilement des variables.
...
```{r}
print(paste("p-valeur =", anova(modselect_bic_back, model_complet)$`Pr(>F)`)[2])
```
...
Nous allons dans un premier temps séparer notre variable `Energy.efficiency` en deux classes (A/B ou C/D/E/F/G) que nous stockerons dans la variable `Energy.efficiency.bis` (1 ou 0).
...
Ainsi, les bâtiments à la meilleure consommation énergétique ont
...
On trouve un pseudo $R^2$ de 0.71, ce qui n'est pas trop mal. Nous allons voir si nous pouvons le simplifier en recherchant un sous-modèle. Pour cela allons réaliser des sélections par critères BIC et AIC à l'aide de la fonction `bestglm`.
...
La p-valeur obtenue est de 0.003229, donc on rejette le sous modèle BIC au risque de 5\%.
...
Ainsi nous retenons donc pour l'instant le modèle AIC.
...
```
On obtient ainsi un pseudo $R^2$ de 0.720, ce qui est un peu plus élevé que pour le modèle complet (0.713). L'augmentation est cependant très faible, et l'ajout d'interactions est peut-être inutile.

On fait un test de sous-modèle afin de voir si le modèle sans interactions BIC précédent ne suffirait pas à expliquer nos données :
...
### Modèle logistique final

Finalement, le modèle de régression logistique que nous retenons est le suivant :
...
Nous avons essayé de faire de la régression polytomique, mais nous n'avons pas eu le temps de s'appesantir dessus. Le code est ainsi présenté dans le fichier `Rmd`, mais n'est pas présenté dans le rapport.
...
<!--   * Les classes A,B,C ont une faible `Relative.compactness` par rapport aux classes D,E,F,G. -->
<!--   * Les classes A,B,C ont une grande `Surface.area` par rapport aux classes D,E,F,G. -->
<!--   * Les classes A,B,C ont principalement une hauteur de 3.5 m, contre plutôt 7 m pour les autres. -->

```{r}
data.mlg.p$Energy.efficiency = factor(data.mlg.p$Energy.efficiency, order = TRUE,
                                      levels = c("A", "B", "C", "D", "E", "F", "G"))
# écart de overall.height car facteur ordonnée => posait des problèmles à vglm
...
Nous allons maintenant essayer de prédire la consommation énergétique à partir de modèles non linéaires. Nous étudierons dans un premier temps des modèles de classification directe, puis des modèles de régression.
...
Nous essayons d'abord de classer nos données en deux catégories (`Energy.efficiency.bis`) par des algorithmes de classification.

### Préparation des données

Pour la réalisation de l'arbre optimal, de la régression logistique et de la forêt aléatoire, nous allons
scinder notre jeu de données en un ensemble d'apprentissage et un ensemble de test. Cela nous permettra
de voir la robustesse de notre modèle face à de nouveaux individus.
...
ratio_train = 0.75
set.seed(2021)
nb_train <- floor(ratio_train * nrow(data))
train_ind <- sample(seq_len(nrow(data)), size = nb_train)
...
Dans la partie modèle linéaire généralisé, nous avions déjà construit un modèle de régression logistique. A partir de ce modèle, nous allons tracer la  table de contingence pour les éléments constituants l'ensemble test. Nous pourrons ainsi comparer ces résultats avec ceux des modèles non-linéaires.
...
print(paste("Pourcentage de bien classés :", pct_reg_log))
```
On retrouve un pourcentage de valeurs correctement prédites de ```r round(pct_reg_log,4)*100```\% pour la régression logistique.

### Arbre binaire de décision

Dans cette partie, nous allons construire un arbre binaire de décision.

#### Version naïve (``cp`` par défaut)

Pour la création de cet arbre, nous allons  utiliser la fonction `rpart` avec les paramètres par
défaut.
...
rpart.plot(tree.dis)
```
L'analyse de l'arbre binaire de décision permet d'avoir une classification en deux catégories selon la variables `Overall.height`. Quand la hauteur vaut 7, aucune autre subdivision n'est faite. Quand la hauteur vaut 3.5, on observe  que une profondeur importante du sous-arbre. Ainsi, notre arbre pourrait manquer de flexibilité pour de nouvelles données.
...
print(paste("Pourcentage de bien classés :", pct_arb_nai_dis))
```

Après avoir tracé la table de contingence on retrouve un pourcentage ```r round(pct_arb_nai_dis,4)*100```\% d'individus bien classés. 

#### Version optimale

L'arbre de décision binaire a pour défaut  de ne pas être flexible pour la prédiction de nouvelles données. Pour pallier ce problème, nous allons rechercher un arbre optimal élagué en pénalisant sa complexité. Nous utilisons pour cela de la validation croisée.
...
```
L'arbre optimal obtenu comporte bien moins de feuilles que ce celui non optimal, ce qui est un signe d'une structure plus flexible, car plus générale.

```{r}
pred.tree.opti <- predict(tree.dis.opti,newdata=test.dis,type="class")
...
print(paste("Pourcentage de bien classés :", pct_arb_opti_dis))
```
Après avoir réalisé la table de contingences sur les données testées, on observe ```r round(pct_arb_opti_dis,4)*100```\% d'individus bien classés, ce qui est en effet supérieur à la valeur trouvée sur l'arbre non optimisé (moins d'over-fitting).

### Random forest

Dans cette partie nous allons construire une forêt aléatoire, en vue de prédire l'appartenance d'un individu à une classe. En effet, nous avons vu précédemment que l'arbre optimal pouvait être instable par rapport aux données d'apprentissage. Ainsi, grâce au bagging et au rééchantillonnage des variables qu'implémentent les forêts aléatoires, nous devrions avoir une plus grande stabilité de notre algorithme.

#### Version naïve (``mtry`` par défaut)

Nous allons commencer par la création d'une random forest avec les paramètres par défaut de la fonction `randomForest`, avant de tester ses performances sur notre jeu de test.
...
print(paste("Pourcentage de bien classés :", pct_rdm_nai_dis))
```
On observe pour les paramètres par défaut du random forest un pourcentage de bien classés de ```r round(pct_rdm_nai_dis,4)*100```\%.

#### Version optimale

Dans cette version, on cherche à optimiser le paramètre `mtry` qui permettrait d'avoir le meilleur arbre en minimisant l'erreur OOB (out of bag). Ce paramètre représente le nombre de variables sélectionnées pour chaque arbre de la forêt.
...
On peut ensuite visualiser l'importance de chaque variable en traçant les graphes d'importance.
On peut ainsi quantifier l'apport de chaque variable à la construction de l'arbre.

```{r, fig.dim=c(8,3.5), fig.margin=F}
par(oma = c(0,0,0,0))
varImpPlot(rf.dis.opti)
```

Les variables contribuant le plus à la diminution de la précision et de l'indice de Gini sont celles qui sont les plus influentes. Ainsi les variables concernant la forme des bâtiment sont encore les plus importante, ensuite viennent les variables concernant la surface vitrée, puis finalement l'orientation qui n'a que très peu d'effet. Ces conclusions sont cohérentes avec toutes nos observations jusqu'à présent.
...
Ici le pourcentage trouvé est un peu plus faible que pour l'arbre naïf, alors que l'on s'attendrait à trouver le contraire. Cela est peut-être dû aux ensembles de test et d'apprentissage. Les deux valeurs sont de toute façon du même ordre de grandeur, et les deux modèles semblent performants.

## Régression

Dans cette partie, nous allons réaliser les arbres précédents en mode  régression en vue de faire la comparaison aux résultats obtenues pour celle de la classification.

### Préparation des données

Nous allons aussi travailler sur données d'apprentissage et des données tests.
...
print(paste("Pourcentage de bien classés :", pct_arb_nai_reg))
```
On observe pour l'arbre de régression naïf un pourcentage de bien classés de ```r round(pct_arb_nai_reg,4)*100```\%.
...
On obtient pour l'arbre optimal exactement le même arbre que nous avions précédemment, il était donc déjà optimal.
...
print(paste("Pourcentage de bien classés :", pct_rdm_nai_reg))
```

On observe un pourcentage de bien classés de ```r round(pct_rdm_nai_reg,4)*100```\%.
...
pct_rdm_opti_reg
print(paste("Pourcentage de bien classés :", pct_rdm_opti_reg))
```

On observe un pourcentage de bien classés pour la forêt optimale de ```r round(pct_rdm_opti_reg,4)*100```\%.
...
Nous avions également vu que `Overall.height` séparait bien nos données vis-à-vis de l'énergie, ce qui explique qu'elle ait été sélectionnée. De même, la présence/absence de vitres était également assez discriminante (modalité 0 contre modalités 1 à 5 de `Glazing.area.distr`).

D'après le graphique de l'importance des variables de la Random Forest, on retrouve la même hiérarchisation de la contribution des variables, à savoir la forme du bâtiment (deux premiers axes de l'ACP), puis la surface vitrée (troisième axe).

# Conclusion

Nous avons ainsi pu mettre en place plusieurs méthodes afin de prédire la consommation énergétique des bâtiments. Les résultats obtenus varient d'un modèle à l'autre, mais certaines tendances se dégagent tout de même.

D'abord, la variable `orientation` ne sert jamais à rien : tous les modèles l'ont laissée de côté. La consommation d'énergie n'est donc pas lié à l'orientation du bâtiment.

L'influence la plus importante sur la consommation semble être clairement liée à la forme du bâtiment. Ainsi, nos modèle conservent toujours les variables `Relative.compactness` et `Overall.height`. `Surface.area` et `Wall.area` sont aussi souvent conservées, mais il y a de toute manière un lien entre toutes ces variables, ce qui fait qu'elles sont parfois laissées de côté.

Finalement, la surface vitrée semble aussi jouer un rôle, même si celui ci est moins important que la forme. Ainsi, les bâtiments moins vitrés consomment moins, ce qui paraît cohérent.

Pour ce qui est des performances des modèles, nous les avons comparé sur les mêmes jeux d'entraînement et de test. Comme précédemment, nous n'avons comparé que les régressions, en seuillant la consommation à 35 (classes A/B contre C/D/E/F/G).

```{r, echo=F, eval=T}
final_tree   = tree.reg
final_forest = rf.reg.opti

test = data[-train_ind,allvariables]

print("Pourcentage de bien classés (deux classes)")

# regression lineaire
final_linear_model = lm(modselect_bic_back$terms, data = data[train_ind,])
test_linear = predict(final_linear_model, test)
pct_linear = prct_bien_classe(table(test_linear > 35., test[,"Energy"] > 35.))
print(paste("Régression linéaire :  ", pct_linear))

# regression logistique
print(paste("Régression logistique :", pct_reg_log))

# arbre de regression
print(paste("Arbre de régression :  ", pct_arb_opti_reg))

# foret de regression
print(paste("Forêt aléatoire :      ", pct_rdm_opti_reg))
```

Ainsi, le modèle linéaire classique est le moins performant. Nous avions en effet observé un décrochage sur le QQ-plot, qui supposait que ce type de modèle n'était pas très adapté.

La régression logistique est meilleure, mais ce sont finalement les arbres et la forêt aléatoire qui prédisent le mieux la consommation énergétique.

Les modèles non-linéaires se sont donc finalement révélés meilleurs que les modèles linéaires pour cette étude.
