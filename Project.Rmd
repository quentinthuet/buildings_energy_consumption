...
quali = c(5, 6, 8, 10)
```
La commande summary nous permet  de noter qu'il y a
le même nombre de bâtiments pour chaque orientation, pour chaque hauteur.
...
```
La courbe précédente nous montre que les variables `Roof.area`, `Wall.area` et `Surface.area` sont liées par la relation :
...
# tau_seq <- seq(0, 0.005, by = 0.0000001)
...
gg = ggplot(dfridge,aes(x=tau,y=theta,col=variable)) +
  geom_line() +
...
gglasso = ggplot(dflasso,aes(x=tau,y=theta,col=variable)) +
  geom_line() +
...
```{r}
data.nlm.class = data.mlg[c(1,2,3,4,5,6,7,13)]
...
table(data.mlg[-train_ind,]$Energy.efficiency.bis, hatY[-train_ind])
```
...
table(pred.tree, test[, "Energy.efficiency.bis"])
```
...
table(pred.tree.opti, test[, "Energy.efficiency.bis"])
```

```{r}
library(randomForest)
rf.dis <- randomForest(Energy.efficiency.bis ~ ., data = train, 
                       xtest = test[, -8], ytest = test[, "Energy.efficiency.bis"],
...
rf.dis.opti <- randomForest(Energy.efficiency.bis ~ ., data = train, 
                       xtest = test[, -8], ytest = test[, "Energy.efficiency.bis"],
                       ntree = 500, do.trace = 50, importance = TRUE,mtry=as.integer(rfFit$bestTune))
pred.rf.opti <- rf.dis.opti$test$predicted
table(pred.rf.opti, test[,"Energy.efficiency.bis"])
```

## Régression
```{r}
data.nlm.reg = data[c(1,2,3,5,6,7,8,9)]
...
library(rpart) # chargement de la librairie
library(rpart.plot)
tree.dis.reg=rpart(Energy ~.,data=train, cp=0.001)
rpart.plot(tree.dis.reg)
```
