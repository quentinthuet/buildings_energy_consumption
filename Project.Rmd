...
Puisque, les variables Surface.area, Wall.area et la Roof.area sont linéairement liées, nous allons enlever une de ces variables (Roof.area) du jeu de données.
...
donc nous allons considérer le modèle additif pour commencer notre étude.
...
print(pseudoR2)
```
Le Pseudo R2 trouvé est de 0.712, est assez proche de 1, alors on peut modéliser ce problème par un MLG.
...
La pvaleur obtenue étant de 0.003229  donc on rejette le sous modéle AIC au risque de 1%. 
...
#### Conclusion  :
L'analyse des sous modéles fournis par la fonction bestglm  (avec comme critére AIC et BIC) de R nous a permis de retenir le 
sous modèle suivant (celui du BIC) :Energy.efficiency.bis ~ Relative.compactness + Glazing.area + 
...
Pour cela nous allons considérer les intéractions entre Surface.area et Relative.compactness, entre Wall.area et Glazing.area et
entre Surface.area et Wall.area. Pour des questions de performances de l'algorithme nous avons choisies de ne pas considérer toutes les intéractions.
```{r}
...
print(pseudoR2)
```
Le pseudo R2 obtenu est de 0.7194924 (contre  0.712583 pour le modèle additif), il est meilleur mais à l'ordre de 10e-3
...
La pvaleur étant de 0.3212 on ne rejette donc pas H0
ON conserve le sous  modèle AIC
## Choix du meilleur sous modèle entre celui avec intéraction et sans intéractions
Comparaison du modèle sans intéraction avec le sous modéle avec intéraction 
Dans ce cas précis les modèle sans intéraction est un sous modèle de celui avec intéraction donc
on peut réaliser un test de sous modèle de "Chisq" de R.
```{r}
anova(mlg_rg_best_bic,mlg_inter_best,test="Chisq")
```

La pvaleur étant de 0.18 donc on conserve le modéle sans intéraction.
suivant :
 Energy.efficiency.bis ~ Relative.compactness + Glazing.area + 
    Overall.height + Glazing.area.distr
...
donc nous devrions considérer le modèle additif avec toutes les autres variables.
