...
Nous allons d'abord construire un modèle linéaire généralisé additif sans interactions.

### Ajustement du modèle

```{r, echo=F}
mlg_rg <- glm(Energy.efficiency.bis~.,data=data.mlg,family=binomial(link=logit)) ; #summary(mlg_rg)
```

### Calcul du pseudo R2

```{r}
pseudoR2 = 1 - ( mlg_rg$deviance/mlg_rg$null.deviance) ; print(paste("Pseudo R² :",pseudoR2))
```
On trouve un pseudo $R^2$ de 0.71, ce qui n'est pas trop mal. Nous allons voir si nous pouvons simplifier 
